...
pub fn compare_reports(before: &AgentReport, after: &AgentReport) -> DiffReport {
    ...
    // --- security.access_alignment.root_equivalent_groups (R33-QW-7, R34-02) ---
    //
    // All inventoried groups are tracked, sudo/wheel/shadow included:
    // R34-01 makes the point-in-time weight flat per class, so a principal
    // *gaining* membership between snapshots is what separates a known ops
    // account from persistence (`usermod -aG docker|sudo`).
    //   member added (incl. first member / new group) → Degraded
    //   member removed, group emptied, group absent   → Changed
    // Removal is never Improved: indistinguishable from post-use cleanup and
    // from an unreadable /etc/group (R34-03).
    {
        ...
        for (name, now) in &a {
            let was = b.get(name).unwrap_or(&none);
            let mut plus = now.difference(was).peekable();
            if plus.peek().is_some() {
                grew.push(format!("{name} +{}", join(plus)));
            }
            let mut minus = was.difference(now).peekable();
            if minus.peek().is_some() {
                lost.push(format!("{name} -{}", join(minus)));
            }
        }
        for (name, was) in b.iter().filter(|(n, _)| !a.contains_key(*n)) {
            lost.push(if was.is_empty() {
                format!("{name} (group absent)")
            } else {
                format!("{name} -{} (group absent)", join(was.iter()))
            });
        }

        const FIELD: &str = "security.access_alignment.root_equivalent_groups";
        if !grew.is_empty() {
            changes.push(Change {
                field: FIELD.into(),
                before: None,
                after: Some(grew.join("; ")),
                severity: Severity::Degraded,
            });
        }
        if !lost.is_empty() {
    ...
    // --- security.kernel_taint (SEC-038 drift) ---
    {
        ...
        if newly_set != 0 {
            let names: Vec<String> = after
                ...
                .flags
                .iter()
                .filter(|f| newly_set & (1u64 << f.bit) != 0)
                .map(|f| format!("{} ({})", f.name, f.code))
                .collect();
            changes.push(Change {
                field: "security.kernel_taint".into(),
                before: Some(format!("tainted={before_bits}")),
                after: Some(format!("newly tainted: {}", names.join(", "))),
                severity: Severity::Degraded,
            });
        }
    }
    ...
    // --- security.confinement (SEC-039 drift) ---
    {
        ...
        let after_c: HashSet<&str> = after
            ...
            .map(|p| p.profile.as_str())
            .collect();
        for downgraded in after_c.difference(&before_c) {
            changes.push(Change {
                field: "security.confinement.apparmor".into(),
                before: None,
                after: Some(format!(
                    "profile {downgraded} now in complain mode (was enforcing or new)"
                )),
                severity: Severity::Degraded,
            });
        }
        for restored in before_c.difference(&after_c) {
            changes.push(Change {
                field: "security.confinement.apparmor".into(),
                before: Some(format!("profile {restored} was in complain mode")),
                after: None,
                severity: Severity::Improved,
            });
        }
        if after.security.confinement.selinux_permissive
            && !before.security.confinement.selinux_permissive
        {
            changes.push(Change {
                field: "security.confinement.selinux".into(),
                before: Some("enforcing".into()),
                after: Some("permissive".into()),
                severity: Severity::Degraded,
            });
        }
    }
