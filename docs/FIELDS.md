...
| `mount_namespace_anomalies[].exe_path` | string | Resolved executable path. **Namespace-relative, not a host path**: for a container process this path exists inside the container's mount namespace, not on the host. To inspect: `sudo nsenter --mount=/proc/<pid>/ns/mnt ls -l <path>`. Never `null`: processes whose `/proc/<pid>/exe` cannot be resolved (kernel worker threads, zombies) are omitted from the array entirely. |
